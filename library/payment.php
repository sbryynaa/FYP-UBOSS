...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['pay_now'])) {
    ...
    $sql = "UPDATE tblissuedbookdetails SET pay_status = :pay_status WHERE id = :issued_id";
...
if (isset($_GET['issued_id'])) {
    ...
    $sql = "SELECT amount FROM tblissuedbookdetails WHERE id = :issued_id";
    $query = $dbh->prepare($sql);
    $query->bindParam(':issued_id', $issued_id, PDO::PARAM_INT);
    $query->execute();
...
?>
<style>/* Adjusting the form container */
.panel {
    border-radius: 10px;
    border: 1px solid #ddd; /* Adding border */
    background-color: #ffffff;
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
    max-width: 400px; /* Making the form container smaller */
    margin: 20px auto;
    padding: 20px;
}
...
.receipt {
    ...
    border: 1px solid #ddd; /* Adding border */
    padding: 20px; /* Reducing padding */
    border-radius: 10px;
    margin-top: 30px;
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
    max-width: 400px; /* Making the receipt container smaller */
    margin: 20px auto;
}
        ...
        <div class="container">
            <?php if (isset($payment_success) && $payment_success): ?>
                <div class="receipt">
                    ...
                    <div class="details">
                        <strong>Amount:</strong> RM <?php echo htmlentities($amount); ?>
                    </div>
                    <div class="details">
                    ...
                    <div class="panel-body">
                        <form method="POST" action="payment.php">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="amount_display" value="RM <?php echo htmlentities($amount); ?>" readonly>
                            </div>
